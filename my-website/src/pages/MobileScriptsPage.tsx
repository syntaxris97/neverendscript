...
const MobileScriptsPage: React.FC = () => {
  const [currentPage, setCurrentPage] = useState(1);
  const [searchQuery, setSearchQuery] = useState('');

  // Filter scripts to only show mobile scripts
  const mobileScripts = scriptData.filter(script => script.category.includes('Mobile'));
  
  // Further filter based on search query
  const filteredScripts = searchQuery
    ? mobileScripts.filter(script => 
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-6">Mobile Scripts</h1>
        ...
        <div className="mb-6">
          ...
          {filteredScripts.length === 0 && (
            <p className="text-gray-600 mt-2">
              No mobile scripts found. Try a different search term.
            </p>
          )}
        </div>
      )}
...
export default MobileScriptsPage;